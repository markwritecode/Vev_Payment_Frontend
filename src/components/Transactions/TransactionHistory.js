    ...
    return (
        <div className='mt-[22px]'>
            <div className='md:flex items-center justify-between text-center md:text-left'>
                ...
                <button onClick={toggleCreateTransaction} className='flex items-center gap-2 bg-[#F3724F] px-[10px] py-[5px] rounded-[5px] text-white mx-auto md:mx-0'>
            ...
            <div className='mt-[26px] bg-white pt-[22px]'>
                <div
                    className='relative text-gray-400 focus-within:text-gray-400 w-full md:w-96 mx-auto px-3 md:px-0'
                ...
                <div className='mt-[23px] divide-y divide-black divide-opacity-30 overflow-x-auto'>
                    {
                        data?.transactions?.map((item, index) => {
                            ...
                            return (
                                <div key={index} className='md:flex items-center justify-between py-[15px] md:pl-[20px] md:pr-[53px]'>
                                    <div className='md:flex items-center gap-[12px]'>
                                        <img src={`/images/avatar${i + 1}.png`} className='h-[50px] w-[50px] rounded-full mx-auto md:mx-0' alt='vev' />
                                        <div className='text-center md:text-left'>
                                            <h4 className='font-semibold text-sm md:text-base text-black opacity-60'>{item.owner}</h4>
                                            <h5 className='text-xs md:text-sm text-black opacity-60'>{dateFormatter(item.created_at)}</h5>
                                        </div>
                                    </div>
                                    ...
                                    <div className='font-semibold text-sm md:text-base text-black text-opacity-50 pl-2 md:pl-0'>${currencyFormatter(item.amount)}</div>
                                </div>
                            )
                        })
                    }
