...
const StepTwo = ({ responseData }) => {

    const [signupForm] = Form.useForm()
    const navigate = useNavigate()
    const { mutate, isLoading } = usePoster(endpoints.USER_CREATE, 'User created successfully, please sign in.', [], () => navigate(urls.un_auth.LOGIN))
    ...
    useEffect(() => {
        signupForm.setFieldsValue({ 'email': responseData.email })
    }, [responseData])
        ...
        >
            <div className='px-5 py-10 sm:p-10 border-[1px] border-slate-200 shadow-lg rounded-md flex flex-col items-center space-y-4'>
                ...
                <Form.Item
                    ...
                    <Input className='p-3 border-[1px] border-slate-500 rounded-sm xs:w-80 focus:outline-none focus:border-[#1eabe7e3]' placeholder='Verification Code' />
                </Form.Item>
